				...
				this.timer = setTimeout(()=>{
					...
					for (let i in this.phones){
						this.phones[i].forEach((item)=>{
							// console.log(item,item.spell,item.name);
							if(typeof item.spell!='undefined'||typeof item.name!='undefined'){
								let _keyword = this.keyword;
									_keyword.toLowerCase();
								if(item.spell.indexOf(_keyword) > -1||item.name.indexOf(_keyword) > -1){
									result.push(item)
								}
							}
