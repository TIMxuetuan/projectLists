<template>
	<view class="container">
		...
		<button type="primary" @tap="sendData" :disabled="disabled">确认修改</button>
...
<script>
	...
	export default {
		...
		onLoad(options) {
			...
			this.getTags()
			const _data = uni.getStorageSync('competitor_detail');
			this.model = JSON.parse(_data)
			// 拿到数据后，清除缓存
			try {
			    uni.removeStorageSync('act_detail');
			} catch (e) {
			    // error
			}
		...
		methods:{
			sendData(){
				...
				Service.editCompetitor(this.competitorId, _data).then(res =>{
					if(res.Flag){
						uni.showToast({
							title:'修改成功',
							icon:'none'
