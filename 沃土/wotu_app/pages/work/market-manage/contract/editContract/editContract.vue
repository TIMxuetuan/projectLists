<template>
	<view class="container">
		...
		<button type="primary" class="btn" :disabled="disabled" @tap="sendData">确认修改</button>
		...
		methods:{
			...
			getData(){
				Service.contractDetail(this.contractId).then(res =>{
					if(res.Flag){
						...
						this.model = {
							name:_data.Name,
							totalPrice:_data.Price,
							discount:_data.Sale,
							signDate:_data.Date.Sign,
							startDate:_data.Date.Start,
							endDate:_data.Date.End,
							signer:_data.CustSigner,
							contractNo:_data.CtrnNo,
							content:_data.Text,
							introduction:_data.GoodsDesc,
							description:_data.Desc,
							chance:{label:_data.Chance.Name,value:_data.Chance.Id},
							customer:{label:_data.Customer.Name,value:_data.Customer.Id},
							payment:{label:_data.PayType.Name,value:_data.PayType.Id},
							contractState:{label:_data.State.Name,value:_data.State.Id},
						}
						if(_data.Tags.length !== 0){
							this.model.contractType = {
								label:_data.Tags[0].ItemValue,
								value:_data.Tags[0].ItemId
							}
						}
						// 根据OwenerId SignerId查找对应的name
						Service2.getMembers().then(res =>{
							if(res.Flag){
								this.tagList5 = res.Content.map(item =>{
									if(_data.OwnerId === item.Id){
										this.model.principal = {
											label:item.Name,
											value:item.Id,
										}
									}
									if(_data.SignerId === item.Id){
										this.model.mySigner = {
											label:item.Name,
											value:item.Id,
										}
			...
			onConfirm(e, type){
				if(type === 0){
					...
					console.log(this.model.principal)
				}else if(type === 5){
					this.model.mySigner = e.checkArr;
					console.log(this.model.mySigner)
				}else if(type === 6){
			...
			sendData(){
				...
				Service.editContract(this.contractId, _postData).then(res =>{
					if(res.Flag){
						uni.showToast({
							title:'修改成功',
							icon:'none',
