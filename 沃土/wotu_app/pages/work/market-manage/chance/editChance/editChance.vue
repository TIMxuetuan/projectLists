<template>
	<view class="container">
		...
		<button type="primary" @tap="sendData" :disabled="disabled">确认更改</button>
...
<script>
	...
	export default {
		...
		onLoad(options) {
			...
			// 接收详情页传过来的数据,不建议这样写,如果对象内容太多,会发生异步阻塞,
			// 导致这一步没执行完，就执行下一步，所以下边加了定时器
		...
		methods:{
			...
			sendData(){
				...
				Service.editChance(this.model.id, _postData).then(res =>{
					if(res.Flag){
