...
<script>
	...
	export default {
		...
		onLoad(options) {
			let _data = uni.getStorageSync('order_data');
			this.model = JSON.parse(_data)
			this.cur_products = this.model.products;
			uni.removeStorageSync('order_data');
		...
		methods:{
			sendData(){
				...
				Service.editOrder(this.oId,_postData).then(res =>{
					if(res.Flag){
						uni.showToast({
							title:'创建成功',
							icon:'none'
