...
export default {
  ...
  data() {
    return {
      ...
      res: {},
      TableHeader: [
        { type: "pi_name_rp", label: "奖罚名称" },
        { type: "pi_time_rp", label: "奖罚时间" },
        { type: "pi_unit_rp", label: "奖罚单位" },
        { type: "pi_reasons_rp", label: "奖罚原因" }
      ],
      bigLists: [],
      deleteIndex: "",
      methodList: ["pi_name_rp", "pi_time_rp", "pi_unit_rp", "pi_reasons_rp"],
      show: false,
      isCompile: true,
      isDelete: true
    };
  ...
  methods: {
    ...
    getDatas() {
      this.$get("/getApi").then(res => {
        console.log(res);
        if (res.Flag) {
          this.res = res.data.rewardsPunishments;
          this.chuLiDatas();
        } else {
          Toast("请求错误");
        }
