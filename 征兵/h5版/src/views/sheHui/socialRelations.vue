...
export default {
  ...
  data() {
    return {
      leftZhi: true,
      allJingli: "主要社会关系成员情况",
      res: {},
      TableHeader: [
        { type: "pi_appellation_mm", label: "称谓" },
        { type: "pi_fullName_mm", label: "姓名" },
        { type: "pi_IDNumber_mm", label: "身份证号" },
        { type: "pi_unitPosition_mm", label: "工作单位及职务" }
      ],
      bigLists: [],
      deleteIndex: "",
      methodList: [
  ...
  methods: {
    ...
    getDatas() {
      this.$get("/getApi").then(res => {
        console.log(res);
        if (res.Flag) {
          this.res = res.data.majorSocialRelationsMembership;
          this.chuLiDatas();
        } else {
          Toast("请求错误");
        }
